...
##        swatAt.next = None
    ...
    def check_LOS(self):
        ...
        for swatindex, swat in enumerate(self.swatPos):
            if self.swatPos[swatindex] != None and self.swatPos[(swatindex+1)%NUM_SWAT] != None:
        ...
        for enemyindex, enemy in enumerate(self.enemyPos):
            if self.enemyPos[enemyindex] != None and self.enemyPos[(enemyindex+1)%NUM_ENEMY] != None:
        ...
        for swatindex in range(NUM_SWAT):
            for enemyindex in range(NUM_ENEMY):
                 if duelMatrix[swatindex][enemyindex] == 1:
                     if self.swatPos[swatindex] != None and self.enemyPos[enemyindex] != None:
                        self.markLOS(swatindex, enemyindex)
                        self.shoot(swatindex, enemyindex)

    def markLOS(self, swatindex, enemyindex):
        swat = self.swatPos[swatindex]
        enemy = self.enemyPos[enemyindex]
    ...
    def shoot(self, swatindex, enemyindex):
        ...
        if rand_num <= self.enemyAgents[enemyindex].strength:
            ...
            if (self.swatPos.count(None) == NUM_SWAT):
                self.gameEnd('TERRORISTS WIN')
